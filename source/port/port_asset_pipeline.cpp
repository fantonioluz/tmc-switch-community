...
std::filesystem::path RelativeAssetPath(const std::filesystem::path& path, const std::filesystem::path& root) {
#ifdef __SWITCH__
    // Both paths come from the same directory traversal. Avoid canonicalizing
    // sdmc: paths: libstdc++ canonicalization can fail on the Switch mount.
...
nlohmann::json BuildSourceState(const std::filesystem::path& sourceRoot) {
    ...
    for (const auto& dirent : std::filesystem::recursive_directory_iterator(sourceRoot)) {
        ...
        e.relative = RelativeAssetPath(dirent.path(), sourceRoot).generic_string();
...
bool CopyRuntimePassthroughAssets(const std::filesystem::path& sourceRoot, const std::filesystem::path& outputRoot,
    ...
    for (const auto& relativeDir : kDirectories) {
        ...
        for (const auto& entry : std::filesystem::recursive_directory_iterator(sourceDir)) {
            ...
            const std::filesystem::path relativePath = RelativeAssetPath(entry.path(), sourceRoot);
