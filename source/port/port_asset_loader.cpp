...
const u8* ResolveFragmentedRoomProperty(const std::string& path) {
    if (path.rfind("data_080D5360/", 0) != 0 || gRomData == nullptr)
        return nullptr;

    // Room properties can interleave incbins with .4byte script pointers.
    // A file followed by a numbered continuation is only the first fragment.
    const EmbeddedAssetEntry* entries = EmbeddedAssetIndex_Get();
    const u32 count = EmbeddedAssetIndex_Count();
    const std::string continuation = path.substr(0, path.size() - 4) + "_1.bin";
    for (u32 i = 0; i + 1 < count; ++i) {
        if (path != entries[i].path || continuation != entries[i + 1].path)
            continue;
        const u32 offset = entries[i].offset;
        if (offset < gRomSize && entries[i].size <= gRomSize - offset &&
            entries[i + 1].offset < gRomSize) {
...
bool BuildAreaFromAssets(u32 area) {
    ...
    for (size_t room = 0; room < jsonTables.size(); ++room) {
        ...
        for (size_t i = 0; i < propertySlots; ++i) {
            ...
            if (const u8* complete = ResolveFragmentedRoomProperty(roomEntry.files[i])) {
                props[i] = const_cast<u8*>(complete);
                continue;
            }
