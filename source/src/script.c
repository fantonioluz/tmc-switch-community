...
extern void sub_0806200C(Entity*);
#else
...
void ScriptCommand_Call(Entity* entity, ScriptExecutionContext* context) {
    ...
    Port_DialogTrace_Call(entity, context, gba_addr, (void*)func);
    if (gba_addr == 0x0806200D) {
        sub_0806200C(entity);
        Port_DialogTrace_Result(entity, gba_addr, (void*)sub_0806200C);
        return;
    }
    if (func) {
        func(entity, context);
...
void ScriptCommand_CallWithArg(Entity* entity, ScriptExecutionContext* context) {
    ...
    Port_DialogTrace_Call(entity, context, gba_func, (void*)func);
    if (func) {
        func(entity, context);
    } else {
...
void ScriptCommand_ShowNPCDialogue(Entity* entity, ScriptExecutionContext* context) {
    ...
    *(u32*)&dia = *(const u32*)rawDia;              /* bitfield: flag, flagType, type, fromSelf */
    if (dia.type == DIALOG_CALL_FUNC) {
        u32 gbaFunc = *(const u16*)(rawDia + 4) | ((u32)*(const u16*)(rawDia + 6) << 16);
        dia.data.func = (void (*)(Entity*))Port_LookupScriptFunc(gbaFunc);
    } else {
