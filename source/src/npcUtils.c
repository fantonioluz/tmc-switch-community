...
#ifdef PC_PORT
    Port_DialogTrace_Show(ent, dia->type, (uintptr_t)dia->data.func);
#endif
    temp = *(((u16*)dia) + 1);
    fromSelf = (temp >> 4) & 1;
    temp &= 0xf;
...
#ifdef PC_PORT
            {
                uintptr_t funcValue = (uintptr_t)dia->data.func;
                if (funcValue >= 0x08000000u && funcValue < 0x0A000000u) {
                    ((Dialog*)dia)->data.func = (void (*)(Entity*))Port_LookupScriptFunc((u32)funcValue);
                }
            }
#endif
            if (dia->data.func != 0) {
