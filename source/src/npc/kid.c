...
static const SpriteLoadData* Kid_GetSpriteLoadData(u32 type) {
#ifdef PC_PORT
    if (type >= ARRAY_COUNT(gUnk_0810BDC4))
        return NULL;
#endif
    return gUnk_0810BDC4[type];
}
...
void sub_080620F4(KidEntity* this) {
    if (LoadExtraSpriteData(super, Kid_GetSpriteLoadData(super->type))) {
...
void Kid_Fusion(Entity* this) {
    u32 tmp;
    if (this->action == 0) {
        if (LoadExtraSpriteData(this, Kid_GetSpriteLoadData(this->type))) {
