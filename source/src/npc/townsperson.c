...
#ifdef PC_PORT
extern const u32 gTownspersonSpriteLoadDataCount;
#endif

static const SpriteLoadData* Townsperson_GetSpriteLoadData(u32 type) {
#ifdef PC_PORT
    // Lake Hylia can reach this code with type 64; the table has only 21 entries.
    if (type >= gTownspersonSpriteLoadDataCount)
        return NULL;
#endif
    return gUnk_0810B6EC[type];
}
...
void sub_08061BC8(Entity* this) {
    if (LoadExtraSpriteData(this, Townsperson_GetSpriteLoadData(this->type))) {
...
void sub_08061CEC(TownspersonEntity* this) {
    ...
    if (LoadExtraSpriteData(super, Townsperson_GetSpriteLoadData(super->type))) {
        super->action = 1;
